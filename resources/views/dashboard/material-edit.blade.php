    ...
    <div class="mx-auto max-w-6xl px-6">
        <header class="hero-upload p-6 mb-6 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                @if($user)
                    <img src="{{ $userPhoto ?: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=100&q=80' }}" alt="avatar" class="h-10 w-10 rounded-full object-cover"/>
        ...
        <main class="card-upload p-6">
            @if(session()->has('error'))
                <div class="mb-4 p-4 rounded bg-red-100 text-red-700">{{ session('error') }}</div>
            @endif

            <h2 class="text-lg font-semibold text-slate-800 mb-4">Edit Material Form</h2>

            <form action="/materials/{{ $material->id }}/update" method="post" enctype="multipart/form-data" class="space-y-6">
                {{ csrf_field() }}
...
@foreach($categories as $cat)
                                <option value="{{ $cat->kategori_id ?? '' }}" {{ ($material->kategori_id == ($cat->kategori_id ?? '')) ? 'selected' : '' }}>
                                    {{ $cat->nama_kategori ?? $cat->name ?? 'Category' }}
                                </option>
                            @endforeach
@endif
                        </select>
                    ...
                    <div>
                        ...
                        <input name="title" type="text" class="input-rounded" value="{{ $material->judul }}" required />
                    </div>

                    <div class="md:col-span-2">
                        <label class="block mb-2 small-muted">Course Name</label>
                        <input name="course_name" type="text" class="input-rounded" value="{{ $course->nama_course ?? '' }}" placeholder="Update the course name linked to this material..." />
                    </div>

                    <div class="md:col-span-2">
                        <label class="block mb-2 small-muted">Description</label>
                        <textarea name="description" rows="5" class="input-rounded">{{ $material->deskripsi }}</textarea>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="mt-2 small-muted" id="selectedFileText">Current: {{ basename($material->file_materi) }}</div>
                    </div>
                </div>
